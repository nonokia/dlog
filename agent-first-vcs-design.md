...
v0.1 / v0.2 の実装後に本設計書と実コードを突き合わせた際の差分を、source of truth を実態に合わせて明文化する。§11 の確定判断を再検討するものではなく、実装時に選んだ改良・現実解・命名差の記録である。

### 意図的な改良（実装で選択、コード側にも理由をコメント済み）
...
- **チーム共有：`dlog export` / `dlog import` と `record --author`**（OpenSpec change `decision-sharing`、#64）。ローカル専用からの脱却として、**共有単位をシール済み Decision のみに固定**した上で JSONL の往復を追加した。マージアルゴリズムを必要としないのは実装上の事実に依る — §7.2 の append-only を担保しているのは `decision` の BEFORE UPDATE/DELETE トリガーだけで `INSERT` は塞がれておらず、ID は ULID で大域一意なので「知らない id を挿入するだけ」で2つのストアが合流する。よって §6 の「No CRDT, no distributed sync」と矛盾しない（同期エンジンではなく、確定したシリアライズ形式と再生経路である）。

  staging は出口だけでなく**入口も塞ぐ**。import が `staged: true` / binding 欠落のレコードを含むファイルを丸ごと拒否するのは、そうしないと手書きの JSONL が他人のストアの staging に行を注入でき、その人の次の `dlog commit` が何をシールするかをファイルが決められてしまうため（#59 が示した危険のマシンをまたいだ再現）。

  実装で判明した非自明な点が3つ。(1) FK が3本（`decision.supersedes` / `invariant.declared_by` / `task.parent_task_id`）あり `foreign_keys = ON` なので、`--since` の部分エクスポートは**参照閉包**（supersedes チェーン・task 祖先・宣言された invariant）を同梱しないと import 側で FK が発火する。閉包はデータのある export 時に閉じる。`caused_by` は FK が無く、1本の辺から履歴の大半を引きずり込むため追わない — 欠損は `trace` がそこで止まるだけなので、受け取り側が `dangling_caused_by` として事実を返す（§9.1 原則2）。(2) **ID 昇順は FK 順序をほぼ満たすが、依存してはいけない**。参照先は必ず先に存在した = ULID が小さい、というのはミリ秒解像度でのみ成り立ち、同一ミリ秒内の ULID はランダム部分でしか差が無いため順序が入れ替わりうる（実際にテストが確率的に落ちて発覚した）。稀なケースのためにトポロジカルソートを足すのではなく、import のトランザクション内で `PRAGMA defer_foreign_keys = ON` にして FK 検査を COMMIT 時に寄せた。書き込み順は可読性の性質になり、正当性の前提ではなくなる（本当に宙に浮いた参照は従来どおり失敗し、バッチ全体がロールバックする）。(3) `INSERT OR IGNORE` は使わない — CHECK と FK 違反まで握り潰して「静かに短い import」になるため、存在確認 + 素の INSERT にして §8.2 の staged/binding 不変条件違反はエラーとして出す。

  `record --author`（schema v3、`decision.agent_author`）は任意フィールド。§7.3 の必須最小主義は崩さず、git `user.email` からも拾わない（#58 でワークスペースルートを git ではなく dlog の概念にしたばかりで、identity で git 依存を戻す理由が無い）。第1層と同時に入れたのは、wire format が `format: 1` の時点でこのフィールドを運ぶ必要があり、後から足すと1つのフィールドのために形式が2つ出回るため。

  JSONL の出力先は stdout ではなく `--out <PATH>`。§9.3 の「1呼び出し = stdout に JSON 文書ちょうど1つ」を export だけ破ると、エージェント側のパースが分岐し、書いた件数を返す場所も無くなる。`import` は位置引数（`-` = stdin）で、stdin には出力契約が無いのでパイプ経路は残る。

  **却下**: OpenTelemetry を採らない — ストレージとしては §9 のクエリ面（2軸照合・superseded 除外）が乗らず retention も合わず、`staged → sealed` という**更新**を append-only な span ストリームで表現できない。**副次チャネルとしても採らない**（#64 で確定）: 得られるのは相関だけで §6 の `conversation_id` と同じ「相互運用フィールド」の域を出ない一方、対価としてシール経路に外部エンドポイント依存が入り、そこは §8.1 が「統合点はコミットの瞬間のみ」と狭く保っている場所である。リモートストア（libSQL / Turso）は別イシューに切り出し、要否は export/import の運用後に判断する（§10.1 より、リモートに置けるのは観測値だけでアンカー解決は必ずローカルに残るという制約付き）。

  併せて README に「`.dlog/` を NFS / SMB / 同期クライアントに置いた共有」を明示的な非推奨として記載した（#64 論点4）。SQLite のロックがそこでは信頼できず、失敗の出方がエラーではなくストア破損であるため。強制はしない（パスのマウント元は確実には判定できず、誤判定で正当な構成を塞ぐ）。

### 現実解・未実装（設計意図は別手段で充足）

- **共有ログの規模対策（関連度による選別）は未着手**（#64 論点5）。チーム全員の判断が入ったログでは `context` / `why` の出力が量に支配されるが、`--budget` / `elided`（#33）も §3 のロールアップ（#63）も**新しさ**で切っており関連度では選んでいない。export/import を運用してから改めて起票する既知のギャップとして残す。
...
- **ID は素の ULID**。§7.4 ドラフトの `dec_` / `tsk_` プレフィックスは付けていない。時系列ソート性は ULID そのもので担保。
- **クエリエンベロープのフィールド名**は実装では `{"kind", "target"}`。§9.3 ドラフトの `{"type", "anchor"}` と意味は同じ。
- **`assumptions` フィールドは無い**。前提は `--declares-invariant`（Invariant として単独クエリ可能）か rationale 本文で表現する。なお §7.4 の確定スキーマドラフト自体に `assumptions` は無く（`declares_invariants` のみ）、§7.3 本文の「`rejected` / `assumptions` は任意」の語が §5 → §7 の更新で取り残された内部不整合でもある。
- **§6「Agent Trace を土台に拡張」は §7 の独自スキーマ確定で実質上書き済み**。実装上の接点は Agent Trace 互換の `conversation_id` フィールドのみ。
