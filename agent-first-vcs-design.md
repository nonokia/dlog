...
- **`dlog task done`（§8.3 の非コード系シール）は未実装**。非コード系のシールは `dlog bind --none` で行い、「サブエージェントは終了時に必ず `bind --none`」をインストラクションテンプレートでルール化することで、サブエージェント判断の永続化という設計意図は満たしている。Task エンティティは `record --task` / `--instruction` で受動生成されるのみで、タスクライフサイクル系コマンドは later work。
